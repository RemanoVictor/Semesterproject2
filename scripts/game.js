...
document.getElementById(
  "player1"
).innerHTML = `<img src="${sessionStorage.getItem("icon")}">`;
document.getElementById("result2").innerHTML = sessionStorage.getItem(
  "playerTwo"
);
document.getElementById(
  "player2"
).innerHTML = `<img src="${sessionStorage.getItem("icon2")}">`;
//Board Game
function rollDice() {
  ...
  if (playerOne.tileNum !== playerTwo.tileNum) {
    document.getElementById(
      "tile" + playerTurn.tileNum
    ).innerHTML = `<p>${playerTurn.tileNum}</p>`;
  } else {
    if (playerOne.turn === true) {
      document.getElementById(
        "tile" + playerTurn.tileNum
      ...
      document.getElementById(
        "tile" + playerTurn.tileNum
      ).innerHTML = `<img src="${playerTwo.token}"/>`;
    } else {
      document.getElementById(
        "tile" + playerTurn.tileNum
      ).innerHTML = `<p>${playerTurn.tileNum}</p>`;
      document.getElementById(
        "tile" + playerTurn.tileNum
      ).innerHTML = `<img src="${playerOne.token}"/>`;
    }
  }
  //dice mechanics
  ...
  if (playerTurn === playerOne) {
    ...
    document.getElementById(
      "player1Update"
    ).innerHTML += `<h2> ${playerTurn.title}</h2> <p> ${playerTurn.title} is on tile number:${playerTurn.tileNum}</p>`;
